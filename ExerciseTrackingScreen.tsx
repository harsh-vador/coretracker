...
const ExerciseTrackingScreen = ({route}) => {
  ...
  const onPoseDetected = (pose) => {
    switch (
      currentExercise // Use currentExercise instead of exercise
    ) {
      case "Squats":
        handleSquats(pose);
        break;
  ...
  const handleSquats = (pose) => {
    ...
    if (
      ...
      console.log(
        `Distance: ${distance}, hasSit: ${hasSit}, hasStand: ${hasStand}`
      );
  ...
  const handlePushups = (pose) => {
    ...
    const sufficientConfidence = keypointsConfidence.every(
      (confidence) => confidence > MIN_KEYPOINT_CONFIDENCE
    );
    ...
    if (sufficientConfidence) {
      ...
      const isInDownPositionNow =
        leftArmAngle >= THRESHOLD_DOWN_MIN - TOLERANCE &&
        leftArmAngle <= THRESHOLD_DOWN_MAX + TOLERANCE &&
        rightArmAngle >= THRESHOLD_DOWN_MIN - TOLERANCE &&
        rightArmAngle <= THRESHOLD_DOWN_MAX + TOLERANCE;
      const isInUpPositionNow =
        leftArmAngle > THRESHOLD_UP - TOLERANCE &&
        rightArmAngle > THRESHOLD_UP - TOLERANCE;
  ...
  const handleLunges = (pose) => {
    ...
    if (
      ...
      if (
        ...
        console.log(
          "Detected standing lunge. Incrementing count:",
          noOfLunges + 1
        );
      } else {
  ...
  const handlePlanks = (pose) => {
    ...
    if (
      leftShoulderConfidence > confidenceThreshold &&
      rightShoulderConfidence > confidenceThreshold
    ) {
      // Allow for fallback if at least one ankle is detected with some confidence
      if (
        leftAnkleConfidence > confidenceThreshold ||
        rightAnkleConfidence > confidenceThreshold
      ) {
        // Calculate the differences in Y-coordinates
        const leftShoulderAnkleDiff =
          leftShoulderY - (leftAnkleY || rightAnkleY);
        const rightShoulderAnkleDiff =
          rightShoulderY - (rightAnkleY || leftAnkleY);

        // Check if the user is in the plank position
        const isCurrentlyPlanking =
          Math.abs(leftShoulderAnkleDiff) < yDifferenceThreshold &&
        ...
        if (isPlanking) {
          ...
          setIsPlanking(false);
          console.log(
            "Plank ended due to insufficient ankle confidence, time:",
            elapsed_time
          );
        }
      }
      ...
      if (isPlanking) {
        ...
        setIsPlanking(false);
        console.log(
          "Plank ended due to insufficient shoulder confidence, time:",
          elapsed_time
        );
      }
    }
